...
parser.add_argument("--epochs", default=10, type=int, help="number of epoches for the model to train")
...
class HamiltonianNetwork(nn.Module):
    """
    Standard feedforward network to learn the Hamiltonian H(x).
    """
...
if DIMENSION == 4:
    ...
    ax1.legend()
    fig1.savefig("results/trajectory2d_2.pdf")
    ...
    ax1.set_ylabel(r"$x$")
    ax1.set_title(f"MSE on the test set: {MSE_loss:.3e}")
    ax1.plot(time_set, sample[:, 0], label="original data")
    ax1.plot(time_set[:-2], prediction[:-3, 0], '--', label="prediction")
    ax1.legend()
    fig1.savefig("results/trajectory1d_2.pdf")

# Save CSV outputs for further analysis
if DIMENSION == 4:
    T_plot = len(sample)
    time_arr = np.array(time_set[:T_plot])
    if len(prediction) > T_plot:
        prediction = prediction[:T_plot]
    elif len(prediction) < T_plot:
        time_arr = time_arr[:len(prediction)]

    true_df = pd.DataFrame({
        'time': time_arr,
        'theta1_true': sample[:, 0], 'theta2_true': sample[:, 1],
        'omega1_true': sample[:, 2], 'omega2_true': sample[:, 3]
    })
    pred_df = pd.DataFrame({
        'time': time_arr,
        'theta1_pred': prediction[:, 0], 'theta2_pred': prediction[:, 1],
        'omega1_pred': prediction[:, 2], 'omega2_pred': prediction[:, 3]
    })
    pred_tensor = torch.tensor(prediction[:-1], dtype=torch.float32, device=DEVICE, requires_grad=True)
    with torch.no_grad():
        H_vals = model.H(pred_tensor).cpu().numpy().flatten()
        S_vals = model.S(pred_tensor).cpu().numpy().flatten()
    dS_dt = np.gradient(S_vals, time_arr[:len(H_vals)])
    x_star_list = []
    x_dot_pred_list = []
    for i in range(len(pred_tensor)):
        x_i = pred_tensor[i:i+1]
        S_i = model.S(x_i)
        x_star_i = autograd.grad(S_i, x_i, grad_outputs=torch.ones_like(S_i), create_graph=False)[0]
        x_dot_i = model(x_i)
        x_star_list.append(x_star_i.detach().cpu().numpy().flatten())
        x_dot_pred_list.append(x_dot_i.detach().cpu().numpy().flatten())
    x_star_arr = np.array(x_star_list)
    x_dot_pred_arr = np.array(x_dot_pred_list)
    prod_dS_dt = np.sum(x_star_arr * x_dot_pred_arr, axis=1)
    hs_df = pd.DataFrame({
        'time': time_arr[:len(H_vals)],
        'H_learned': H_vals, 'S_learned': S_vals,
        'dSdt_fd': dS_dt, 'dSdt_predicted': prod_dS_dt
    })
    os.makedirs("results", exist_ok=True)
    true_df.to_csv("results/true_trajectory.csv", index=False)
    pred_df.to_csv("results/predicted_trajectory.csv", index=False)
    hs_df.to_csv("results/hamiltonian_entropy.csv", index=False)
    with torch.no_grad():
        test_vel_pred = model(test_pos)
        vel_error = (test_vel_pred - test_vel).cpu().numpy()
        vel_error_flat = vel_error.reshape(-1, DIMENSION)
        state_flat = test_pos.cpu().numpy().reshape(-1, DIMENSION)
        error_df = pd.DataFrame(
            np.hstack([state_flat, vel_error_flat]),
            columns=[f'state_{i}' for i in range(DIMENSION)] + [f'vel_error_{i}' for i in range(DIMENSION)]
        )
        error_df.to_csv("results/velocity_errors.csv", index=False)
    print("Diagnostic CSV files saved to results/")
