...
Internal energy e is computed as the accumulated loss of mechanical energy:
    e(t) = E_mech(0) - E_mech(t)
so that total energy E_mech + e is exactly conserved.
"""
...
parser.add_argument("--g",       default=9.81,   type=float, help="gravitational acceleration (m/s^2)")
...
def total_energy(theta1, theta2, omega1, omega2):
    """Total mechanical energy E = T + V (does not include internal energy)."""
...
# Generate mechanical trajectories
data_mech = []
np.random.seed(21)
...
for n in range(args.num):
    ...
    for i in range(args.points):
        ...
        time += args.dt
    data_mech.append(np.array(dataset))
    print(f"{n}/{args.num}", end='\r')

# Compute internal energy as energy loss and add as 6th column
data = []
for traj in data_mech:
    t      = traj[:, 0]
    theta1 = traj[:, 1]
    theta2 = traj[:, 2]
    omega1 = traj[:, 3]
    omega2 = traj[:, 4]
    E_mech = total_energy(theta1, theta2, omega1, omega2)
    E0 = E_mech[0]
    e = E0 - E_mech
    full_traj = np.column_stack((t, theta1, theta2, omega1, omega2, e))
    data.append(full_traj)
...
def validate_trajectory(traj):
    ...
    axes[0].plot(t, E_mech, 'b-', lw=1.5, label='Mechanical (T+V)')
    axes[0].plot(t, e,      'g-', lw=1.5, label='Internal e')
    axes[0].plot(t, E_total,'r--', lw=1.5, label='Total (T+V+e)')
